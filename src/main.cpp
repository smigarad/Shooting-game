...
int main()
{
    ...
    Player* player = new Player (default_position,window->renderer,"resource/spaceship2.png");
    ...
    for (uint i =0; i <4; i++)
    {
        ...
        {
            bullets_enemy[i].emplace_back(new Bullet({600,600},window->renderer,"resource/bullet2.png"));
        }
    }

    for(unsigned int i = 0; i < MAX_BULLETS; ++i)
    {
	bullets.emplace_back(new Bullet({1000,1000},window->renderer,"resource/bullet2.png"));
    }
    ...
    while (run == 1)
    {
        ...
        for(auto e: enemies) if( e->health ==0) e->position.x = -100;
        ...
        for(int i=0; i<20; i++)
        {
            bullets_enemy[0][i]->Draw();
            bullets_enemy[1][i]->Draw();
            bullets_enemy[2][i]->Draw();
            bullets_enemy[3][i]->Draw();
        }
        ...
        for (int i=0; i<4; i++)
        {
            for (auto s: bullets_enemy[i]) s->Update(-dt);
        }
